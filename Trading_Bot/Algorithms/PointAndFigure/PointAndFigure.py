...
def sellStock():
    ...
    contractObject = newContract("AAPL", "NASDAQ")
    orderObject = newOrder("SELL", 1)
...
def calcReversal(): #this function with calculate a trend reversal on a point and figure chart
    ...
    if priceDif > 0 and direction == "False":
        if abs(priceDif) > reversal:
            ...
            #if statement will go here to determine if there is not an active position
