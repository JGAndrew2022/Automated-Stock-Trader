...
dif = indicators.iloc[0]['macd_hist'] - indicators.iloc[1]['macd_hist']
dif_prev = indicators.iloc[1]['macd_hist'] - indicators.iloc[2]['macd_hist']
...
quant = cash/float(ts2_pd.iloc[0]['high'])
print(quant)
class BuySellApp(EWrapper, EClient): #Declare class and override methods that we will need, such as the error and order
    ...
    def buyBit(self):
        ...
        order1.lmtPrice = BuySellApp().price + 20
        order1.totalQuantity = quant
        order1.tif = "Minutes"
        
        self.placeOrder(self.nextOrderId, contract, order1)
        
        atr_file = open('D:\MoneyMaking\Innovation\Trading_Bot\Algorithms\Profit_Taking/ATR_When_Bought.txt','w')
        atr_file.write(str(atr_val))
        atr_file.close() 
